...
----------------------------------------------------------------------------------------------------
Procedure:      Zagros.InsertLocation
Create Date:    2020-09-01 (yyyy-mm-dd)
Author:         Surush Cyrus
Description:    Insert a Location
Call by:        TBD, UI, Add hoc

Steps:          1- Error out if @Name already is in table Zagros.Location
                2- Get Max LocationID by calling SP Zagros.GetMaxLocationID
                3- Error out if @LocationID <= 0
                4- Insert to table Zagros.Location

Parameter(s):   @Name
Usage:          Zagros.InsertLocation @Name = 'TBD1'
****************************************************************************************************
SUMMARY OF CHANGES
...
-- Call SP
EXEC Zagros.GetMaxLocationID @MaxLocationID = @LocationID OUTPUT;

SET @Message = 'LocationID = ' + CONVERT(VARCHAR(10), @LocationID) + ' is the return value from SP Zagros.GetMaxLocationID.';   
RAISERROR (@Message, 0,1) WITH NOWAIT;
EXEC Zagros.InsertHistory @SP = @SP,
   @Status = 'Run',
   @Message = @Message
