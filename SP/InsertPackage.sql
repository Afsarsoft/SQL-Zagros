...
----------------------------------------------------------------------------------------------------
Procedure:      Zagros.InsertPackage
Create Date:    2020-09-01 (yyyy-mm-dd)
Author:         Surush Cyrus
Description:    Insert a Package
Call by:        UI, Add hoc

Steps:          1- Check the @Name for RI issue in Zagros.Package
                2- Get Max PackageID by calling SP Zagros.GetMaxPackageID
                3- Error out if @PackageID <= 0
                4- Insert to table Zagros.Package

Parameter(s):   @Name
                @UnitPrice
                @Note

Usage:          Zagros.InsertPackage @Name = 'Test1',
                                     @UnitPrice = 500,
                                     @Note = NULL
****************************************************************************************************
SUMMARY OF CHANGES
...
-- Call SP
EXEC Zagros.GetMaxPackageID @MaxPackageID = @PackageID OUTPUT;

SET @Message = 'PackageID = ' + CONVERT(VARCHAR(10), @PackageID) + ' is the return value from SP Zagros.GetMaxPackageID.';   
RAISERROR (@Message, 0,1) WITH NOWAIT;
EXEC Zagros.InsertHistory @SP = @SP,
   @Status = 'Run',
   @Message = @Message
