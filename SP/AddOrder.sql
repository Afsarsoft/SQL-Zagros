...
File: AddOrder.sql
----------------------------------------------------------------------------------------------------
Procedure:      Zagros.AddOrder
Create Date:    2020-09-01 (yyyy-mm-dd)
Author:         Surush Cyrus
Description:    Add an order 
Call by:        TBD, UI, Add hoc

Steps:          1- Quit if Zagros.CustomerOrder table is empty 
                2- Error out if there is more than one distinct Email in Zagros.CustomerOrder table
                3- Get the Customer order from Zagros.CustomerOrder table
                4- Check to see if we have the customer 
                    - If we do not have the customer, call SP Zagros.GetMaxCustomerID and
                    SP Zagros.InsertCustomer to add the new customer.
                5- Add the order by calling SP Zagros.InsertOrder and SP Zagros.InsertOrderDetail by
                    looping through table variable @CUstomerOrder
                6- Truncate table Zagros.CustomerOrder

Parameter(s):   None
Usage:          EXEC Zagros.AddOrder
****************************************************************************************************
SUMMARY OF CHANGES
...
BEGIN
    ...
    EXEC Zagros.GetMaxCustomerID @MaxCustomerID = @CustomerID OUTPUT;

    SET @Message = 'CustomerID = ' + CONVERT(VARCHAR(10), @CustomerID) + ' is the return value from SP Zagros.GetMaxCustomerID.';
    RAISERROR (@Message, 0,1) WITH NOWAIT;
    EXEC Zagros.InsertHistory @SP = @SP,
                              @Status = 'Run',
                              @Message = @Message
    ...
    EXEC Zagros.InsertCustomer @CustomerID = @CustomerID,
...
BEGIN
    ...
    BEGIN
        ...
        EXEC Zagros.GetMaxOrderID @MaxOrderID = @OrderID OUTPUT;

        SET @Message = 'OrderID = ' + CONVERT(VARCHAR(10), @OrderID) + ' is the return value from SP Zagros.GetMaxOrderID.';
        RAISERROR (@Message, 0,1) WITH NOWAIT;
        EXEC Zagros.InsertHistory @SP = @SP,
        @Status = 'Run',
        @Message = @Message;
        ...
        EXEC Zagros.InsertOrder @OrderID = @OrderID,
    ...
    EXEC Zagros.InsertOrderDetail @OrderID = @OrderID,
                                  @LocationID = @LocationID,
                                  @PackageID = @PackageID,
                                  @UnitPrice = @UnitPrice,
                                  @Quantity = @Quantity
